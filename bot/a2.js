...
SLACK_APP_TOKEN += "394177-uagu24ao8MLa";
SLACK_APP_TOKEN += "1PJ2tgWBT7WQ";

const SLACK_CHANNEL_ID = "U0805NN15FG";

const web = new WebClient(SLACK_APP_TOKEN);

const filePath = path.join(__dirname, "created-accounts.txt");

let firstNames, lastNames;

fs.readFile("./firstName.txt", "utf8").then((res) => {
  firstNames = res
    .split("\n")
    .map((name) => name.trim())
    .filter((name) => name);
...
fs.readFile("./lastName.txt", "utf8").then((res) => {
  lastNames = res
    .split("\n")
    .map((name) => name.trim())
    .filter((name) => name);
...
const getRandomName = async (mode) => {
  try {
    const names = mode ? firstNames : lastNames;
...
const signup = async (page, emailAddress) => {
  try {
    ...
    console.info("  ########## emailAddress:", emailAddress, PASSWORD);
...
const checkConnect = async (page, emailAddress, PASSWORD, server) => {
  try {
    ...
    if (
      ...
      const logEntry = `${date} ${server} ${emailAddress} P: ${PASSWORD}\n`;
      fs.writeFile(filePath, logEntry, { flag: "a" }, (err) => {
        if (err) {
  ...
  try {
    ...
    const proxyList = proxies
      ...
      .map((proxy) => {
        ...
        // console.log("server, username, password:", server, username, password);
...
const startScript = async () => {
  ...
  while (true) {
    ...
    try {
      ...
      //   });
      // });
      firstName = await getRandomName(0);
      lastName = await getRandomName(1);
      randomNum = randomNumber();

      const emailAddress = `${firstName}.${lastName}${randomNum}@yahoo.com`;
